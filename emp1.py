...
from PyQt4.QtGui import QFileDialog, QPixmap, QTableWidgetItem, QMainWindow, QApplication
import base64, io
import os, sys, bs4.dammit
from bs4 import BeautifulSoup
import glob, json
import requests, urllib2, urllib, cStringIO
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.pushButton.setText(_fromUtf8("Show Face"))
        ...
        QtCore.QObject.connect(self.pushButton, QtCore.SIGNAL(_fromUtf8("clicked()")), self.setfile)
    ...
    def getparams(self):
        shost = {"mode" : self.comboBox.currentText(), "user_type" : self.comboBox_2.currentText(), "name" : self.lineEdit_3.text(),
                 ...
                 "entry_type" : self.comboBox_3.currentText(), "image" : self.label0.text()}
        return shost
    ...
    def setfile(self):
        url = 'http://118.102.206.26/face_detect/media/user/'
        download_folder = "/home/kush/Desktop/FaceTrack/Face/"
        ...
        #     print csvfile.image()
        # print link
        for i, each in enumerate(link[5:]):

            u = each.get('href')
            path = os.path.basename(url + u)
            data = urlopen(url + path).read()
            with open(os.path.join(download_folder, 'img' + str([i]) + ".jpg"), "w+") as f:
                f.write(data)
                image.append(data)
                img = QtGui.QImage()
                img.loadFromData(data)
                self.label0.setPixmap(QPixmap(img))
        # print image
        ...
        # image = []
        # for i in range(10):
        #     with open('/home/kush/Desktop/FaceTrack/' + 'img' + str(i), 'w') as f:
        #         f.write(data)
        #         image.append(f.read())
        #         image = QtGui.QImage()
        #         image.loadFromData(data)
        #         self.label0.setPixmap(_fromUtf8(QPixmap(image)))
        #     print image
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton.setText(_translate("MainWindow", "Show Face", None))
        self.label_8.setText(_translate("MainWindow", "Face of Employee", None))
